      ...
      @if (!selEvent()) {
        <div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap;margin:4px 0 6px">
          ...
          <span style="font-size:13px;color:var(--color-text-secondary)"
            ...
            <span class="mo-mono" style="font-size:12px">mochi.event()</span></span
          >
        </div>
        <div style="margin:12px 0 16px">
          <mo-inline-message tone="info"
            >Events are counted in aggregate and are never tied to an individual
            visitor.</mo-inline-message
          >
        </div>
        @if (state() !== 'ready') {
          ...
          >
            ...
            >
            <span
              >Send your first one with
              <span class="mo-mono" style="font-size:12px">mochi.event('name')</span> and it will
              appear here.</span
            >
          </div>
        } @else {
          <div class="mo-card">
            <div class="mo-grid-table__head" [style.grid-template-columns]="grid">
              <span>Event</span><span style="text-align:right">Total</span
              ><span style="text-align:right">Unique visits</span
              ><span style="text-align:right">Conversion</span>
            </div>
            @for (e of data.events(); track e.id) {
              <button
                type="button"
                class="mo-row-btn mo-grid-table__row"
                [style.grid-template-columns]="grid"
                (click)="open(e.id)"
              >
                <span class="mo-mono" style="font-size:12.5px;color:var(--color-accent)">{{
                  e.id
                }}</span>
        ...
        <h1 class="mo-mono" style="font:600 22px/1.2 var(--font-mono);margin:0 0 18px">
          {{ selEvent()!.id }}
        </h1>
        <div
          class="mo-card mo-metric-strip"
          style="grid-template-columns:repeat(auto-fit,minmax(160px,1fr));margin-bottom:16px"
        >
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px">
          <div class="mo-card" style="padding:14px 8px">
            ...
            @for (r of selEvent()!.pages; track r[0]) {
              <div class="mo-kv-row">
                <span class="mo-mono" style="font-size:12px">{{ r[0] }}</span
                ><span>{{ fmt(r[1]) }}</span>
              </div>
            }
          </div>
...
export class Events {
  ...
  private readonly selId = toSignal(this.route.queryParamMap.pipe(map((p) => p.get('event'))), {
    initialValue: null,
  });
  protected readonly selEvent = computed(
    () => this.data.events().find((e) => e.id === this.selId()) ?? null,
  );
