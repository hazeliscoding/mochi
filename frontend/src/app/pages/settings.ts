          ...
          <div>
            ...
            <div style="display:flex;flex-direction:column;gap:14px">
              <div>
                ...
                <textarea id="set-excluded" class="tr-input" rows="2">
/admin/*&#10;/drafts/*</textarea>
              ...
              <label class="tr-choice" style="align-items:center">
                <input
                  ...
                  class="tr-switch"
                  [checked]="botFilter()"
                  (change)="botFilter.set(chk($event))"
                />
              ...
              <label class="tr-choice" style="align-items:center">
                <input
                  ...
                  class="tr-switch"
                  [checked]="selfFilter()"
                  (change)="selfFilter.set(chk($event))"
                />
          ...
          <div>
            ...
            <div style="display:flex;flex-direction:column;gap:14px">
              ...
              <div>
                ...
                <div class="tr-hint">
                  Country is the default. Region requires enough traffic to stay anonymous.
                </div>
              </div>
              <div style="display:flex;align-items:center;gap:10px;font-size:13.5px">
                ...
                <span style="color:var(--color-text-secondary)"
                  >Always on. Addresses are discarded at ingestion and never stored.</span
                >
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div
              style="display:flex;align-items:center;gap:10px;font-size:13.5px;margin-bottom:12px"
            >
              @if (siteActive()) {
                <mo-status tone="success">Installed</mo-status>
                <span style="color:var(--color-text-secondary)"
                  >Events received in the last 30 days.</span
                >
              } @else {
            ...
            <div style="border:1px solid var(--color-danger);border-radius:6px;padding:14px 16px">
              ...
              >
                <span style="font-size:13.5px"
                  ><span style="font-weight:600;display:block">Delete website</span
                  ><span style="color:var(--color-text-secondary);font-size:12.5px"
                    >Removes the site and all of its analytics from Mochi. This cannot be
                    undone.</span
                  ></span
                >
                <button type="button" class="tr-btn tr-btn--danger" (click)="openDelete()">
                  Delete website
                </button>
              </div>
            </div>
        ...
        <mo-dialog [open]="delOpen()" title="Delete website?" (closed)="delOpen.set(false)">
          <div style="max-width:380px;display:flex;flex-direction:column;gap:14px">
            <p style="margin:0;font-size:13.5px;line-height:1.55">
              This permanently deletes <strong>{{ data.currentSite()!.domain }}</strong> and every
              aggregate Mochi has stored for it. There is no undo and no recovery window.
            </p>
            <div>
              ...
              <input
                ...
                type="text"
                class="tr-input"
                [placeholder]="data.currentSite()!.domain"
                [value]="delConfirm()"
                (input)="delConfirm.set(val($event))"
              />
            </div>
            @if (deleteError()) {
            ...
            <div style="display:flex;justify-content:flex-end;gap:8px">
              <button type="button" class="tr-btn tr-btn--secondary" (click)="delOpen.set(false)">
                Cancel
              </button>
              <button
                type="button"
                class="tr-btn tr-btn--danger"
                [disabled]="delConfirm() !== data.currentSite()!.domain || deleting()"
                (click)="doDelete()"
              >
...
export class Settings {
  ...
  protected readonly tz = linkedSignal(
    () => this.data.currentSite()?.timezone ?? 'America/New_York',
  );
