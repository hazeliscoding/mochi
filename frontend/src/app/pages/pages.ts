...
import {
  ...
  TableColumn,
  fmt,
} from '../core/analytics-data.service';
    ...
    <section>
      @if (!selPage()) {
        <div class="mo-page-head">
          ...
          <button
            type="button"
            class="tr-btn tr-btn--secondary tr-btn--icon"
            aria-label="Reverse sort order"
            title="Reverse sort order"
            (click)="flipSort()"
          >
            <mo-icon
              [name]="sortDir() === -1 ? 'arrow-down-wide-narrow' : 'arrow-up-narrow-wide'"
            />
          </button>
        </div>
        @if (state() !== 'ready') {
        ...
        <h1 style="font:600 24px/1.2 var(--font-display);margin:0 0 2px;letter-spacing:-.01em">
          {{ selPage()!.title }}
        </h1>
        <div
          class="mo-mono"
          style="font-size:13px;color:var(--color-text-secondary);margin-bottom:18px"
        >
          {{ data.site() }}{{ selPage()!.id }}
        </div>
        <div
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px">
          <div class="mo-card" style="padding:14px 8px">
            ...
            @for (r of selPageEvents(); track r.name) {
              <div class="mo-kv-row">
                <span class="mo-mono" style="font-size:12px">{{ r.name }}</span
                ><span>{{ r.val }}</span>
              </div>
            }
            @if (!selPageEvents().length) {
              <div style="padding:2px 10px 6px;font-size:13px;color:var(--color-text-secondary)">
                No custom events fired here in this period.
              </div>
            }
          </div>
        </div>
        ...
        <div style="margin-top:12px;font-size:12px;color:var(--color-text-secondary)">
          Referrer, device, and country breakdowns per page are coming soon.
        </div>
      }
    </section>
  `,
...
export class Pages {
  ...
  private readonly selPath = toSignal(this.route.queryParamMap.pipe(map((p) => p.get('path'))), {
    initialValue: null,
  });
  protected readonly selPage = computed(
    () => this.data.pages().find((p) => p.id === this.selPath()) ?? null,
  );
