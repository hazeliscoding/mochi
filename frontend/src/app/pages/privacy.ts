    ...
    <section style="max-width:760px">
      ...
      @if (state() !== 'ready') {
        ...
        <div class="mo-card" style="padding:16px 18px;margin-bottom:16px">
          ...
          <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:14px">Queried live for {{ data.site() }}. These are facts, not promises.</div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px">
            <div>
              <div style="font-size:22px;font-weight:700;font-variant-numeric:tabular-nums">{{ rawHeld() }}</div>
              <div style="font-size:12.5px;color:var(--color-text-secondary)">raw events held, each deleted {{ priv().rawEventLifetimeDays }} days after it arrives</div>
            </div>
            <div>
              <div style="font-size:22px;font-weight:700">{{ oldestLabel() }}</div>
              <div style="font-size:12.5px;color:var(--color-text-secondary)">oldest daily aggregate on record</div>
            </div>
            <div>
              <div style="font-size:22px;font-weight:700">{{ data.retentionLabel(priv().retention) }}</div>
              <div style="font-size:12.5px;color:var(--color-text-secondary)">current retention setting</div>
            </div>
          </div>
        </div>
        ...
        <div class="mo-card" style="padding:16px 18px;margin-bottom:16px">
          ...
          <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:12px">How long Mochi keeps daily aggregates for this website. Saved the moment you pick one; the Settings page shows the same value.</div>
          <div style="display:flex;flex-direction:column;gap:8px">
            @for (r of data.retentionChoices; track r[0]) {
              <label style="display:flex;gap:10px;align-items:center;cursor:pointer">
                <input type="radio" name="retention" [checked]="retention() === r[0]" (change)="setRetention(r[0])" [disabled]="saving()" style="accent-color:var(--color-accent)" />
                <span style="font-weight:600;min-width:190px">{{ r[1] }}</span>
                <span style="font-size:12.5px;color:var(--color-text-secondary)">{{ r[2] }}</span>
              </label>
            }
          </div>
          <div style="min-height:22px;margin-top:8px;display:flex;align-items:center;gap:10px" aria-live="polite">
            @if (saving()) {
        ...
        <div class="mo-card" style="padding:16px 18px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
          <span style="max-width:480px">
            <span class="mo-card-label" style="display:block;margin-bottom:4px">Your data, portable</span>
            <span style="font-size:13px;color:var(--color-text-secondary)">One zip with a CSV per daily aggregate table plus your goals. Everything Mochi has for this site, ready to leave with you.</span>
          </span>
        ...
        <div class="mo-card" style="padding:16px 18px">
          ...
          <p style="margin:0;font-size:13.5px;line-height:1.55;max-width:560px">Each visit is counted under a hash scoped to a single day: a random salt is mixed in at ingestion and destroyed at the UTC day boundary, never stored. Once the salt is gone the hash cannot be recomputed, so the same visitor on two different days can never be linked. No per-visitor record exists to export, hand over, or leak.</p>
        </div>
      }
    </section>
  `,
...
export class Privacy {
  ...
  protected readonly state = computed(() => {
    if (this.data.stateOf(this.data.sitesRes) === 'ready' && !this.data.currentSite()) return 'empty' as const;
  ...
  protected readonly retention = linkedSignal(() => this.data.privacyRes.value()?.retention ?? '1y');
