...
@Component({
  ...
  template: `
    <section style="max-width:640px;margin:0 auto">
      ...
      <div style="font-size:14px;color:var(--color-text-secondary);margin-bottom:20px">
        Three small steps. No cookie banner required afterwards.
      </div>
        ...
        @for (n of [1, 2, 3]; track n) {
          <div style="flex:1;display:flex;align-items:center;gap:8px">
            <span
              ...
              [style.border]="
                '1px solid ' + (step() >= n ? 'var(--color-accent)' : 'var(--color-border)')
              "
              >{{ step() > n ? '✓' : n }}</span
            >
            <span
              ...
              [style.color]="
                step() === n ? 'var(--color-text-primary)' : 'var(--color-text-secondary)'
              "
              >{{ stepLabels[n - 1] }}</span
            >
      ...
      @switch (step()) {
        ...
        @case (2) {
          <div class="mo-card" style="padding:22px">
            ...
            <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:14px">
              One script tag, under 1 KB, no cookies. Pick your setup:
            </div>
            ...
            <div style="display:flex;justify-content:flex-end;margin-top:18px">
              <button type="button" class="tr-btn tr-btn--primary" (click)="verify()">
                I've installed it
              </button>
            </div>
          </div>
        ...
        @case (3) {
          <div class="mo-card" style="padding:40px 24px;text-align:center">
            @if (receiving()) {
              <div
                style="width:56px;height:56px;border-radius:50%;background:var(--color-success-bg);display:inline-flex;align-items:center;justify-content:center;animation:mo-pop .5s ease-out both"
              >
                <svg
                  width="26"
                  height="26"
              ...
              <div style="font:600 20px var(--font-display);margin-top:14px">
                Mochi is receiving analytics from your website. 🍡
              </div>
              <div
                style="font-size:14px;color:var(--color-text-secondary);margin-top:6px;max-width:400px;margin-left:auto;margin-right:auto"
              >
                Your first visit just arrived. Aggregates will build up over the next few hours.
              </div>
            } @else {
              <div style="font:600 20px var(--font-display);margin-top:4px">
                Waiting for the first pageview
              </div>
              <div
                style="font-size:14px;color:var(--color-text-secondary);margin-top:6px;max-width:400px;margin-left:auto;margin-right:auto"
              >
                Open your website in a browser tab, then check again. The first beacon usually lands
                within seconds.
              </div>
            }
            <div style="margin-top:20px;display:flex;gap:8px;justify-content:center">
              ...
              @if (receiving()) {
                <button type="button" class="tr-btn tr-btn--primary" (click)="finish()">
                  Go to dashboard
                </button>
              } @else {
                <button type="button" class="tr-btn tr-btn--primary" (click)="verify()">
...
export class AddWebsite {
  ...
  protected readonly frameworks = computed(() =>
    this.data.frameworksFor(this.created()?.snippet ?? ''),
  );
  protected readonly fwTabs = Object.keys(this.data.frameworksFor('')).map((k) => ({
    value: k,
    label: k,
  }));
  ...
  protected readonly receiving = computed(() => {
    ...
    return (
      !!id &&
      (this.data.sitesRes.value() ?? []).some((s) => s.site.id === id && s.status === 'active')
    );
  });
