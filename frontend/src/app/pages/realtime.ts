    ...
    <section>
      ...
      @if (state() !== 'ready') {
        ...
        <div class="mo-card" style="padding:22px 24px;margin-bottom:16px;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
          ...
          <span style="font-size:16px">active {{ r.active === 1 ? 'visit' : 'visits' }} in the last 5 minutes</span>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:16px">
          <div class="mo-card" style="padding:14px 16px">
            ...
            <svg viewBox="0 0 300 72" style="width:100%;height:auto;display:block" role="img" aria-label="Pageviews per minute over the last 30 minutes">
              @for (v of r.vals; track $index) {
                <rect [attr.x]="$index * 10" width="7" [attr.y]="68 - barH(v, r.maxVal)" [attr.height]="barH(v, r.maxVal)" rx="1.5" fill="var(--color-accent)" [attr.opacity]="$index === 29 ? 1 : 0.45 + ($index / 29) * 0.4" />
              }
            </svg>
          ...
          <div class="mo-card" style="padding:14px 8px">
            ...
            @if (r.pages.length) {
              ...
              <div style="padding:6px 10px;font-size:13px;color:var(--color-text-secondary)">No one is browsing right now.</div>
            }
          </div>
          <div class="mo-card" style="padding:14px 8px">
            ...
            @if (r.sources.length) {
              ...
              <div style="padding:6px 10px;font-size:13px;color:var(--color-text-secondary)">Quiet for the moment.</div>
            }
          </div>
          <div class="mo-card" style="padding:14px 16px">
            <div class="mo-card-label" style="margin-bottom:8px">Countries</div>
            @for (c of r.countries; track c.name) {
              <div style="display:flex;justify-content:space-between;padding:5px 0;font-size:13px"><span>{{ c.name }}</span><span class="mo-muted mo-num">{{ c.val }}</span></div>
            }
            @if (!r.countries.length) {
              <div style="padding:2px 0 6px;font-size:13px;color:var(--color-text-secondary)">No visits in the last 5 minutes.</div>
            }
            ...
            <div class="mo-split" aria-hidden="true">
              <div [style.width.%]="r.devicePct.desktop" style="background:var(--color-accent)"></div>
              <div [style.width.%]="r.devicePct.mobile" style="background:var(--teal-300)"></div>
            </div>
            <div style="display:flex;gap:16px;margin-top:8px;font-size:13px"><span>Desktop {{ r.devices.desktop }}</span><span class="mo-muted">Mobile {{ r.devices.mobile }}</span><span class="mo-muted">Tablet {{ r.devices.tablet }}</span></div>
          </div>
        </div>
      }
    </section>
