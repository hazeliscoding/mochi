    ...
    <section>
      <div class="mo-page-head">
        ...
        <button
          ...
          (click)="router.navigate(['/add-website'])"
        >
        ...
        <div class="mo-card">
          @for (s of data.sites(); track s.id) {
            <button
              type="button"
              class="mo-row-btn"
              style="display:flex;align-items:center;gap:20px;padding:16px 18px;flex-wrap:wrap"
              (click)="open(s)"
            >
              ...
              <div style="min-width:110px;font-size:13px;color:var(--color-text-secondary)">
                {{ s.active }}
              </div>
