      ...
      <div class="mo-auth__card">
        ...
        <form class="mo-card" style="padding:22px;display:flex;flex-direction:column;gap:14px" (submit)="submit($event)">
          <div>
            ...
            <div style="font-size:13px;color:var(--color-text-secondary);margin-top:2px">Privacy-first analytics for your websites.</div>
          </div>
          <div>
            <label class="tr-label" for="login-email">Email</label>
            <input id="login-email" type="email" class="tr-input" autocomplete="email" [value]="email()" (input)="email.set(val($event))" />
          </div>
          <div>
            <label class="tr-label" for="login-password">Password</label>
            <input id="login-password" type="password" class="tr-input" autocomplete="current-password" [value]="password()" (input)="password.set(val($event))" />
          </div>
          @if (error()) {
            <mo-inline-message tone="danger">{{ error() }}</mo-inline-message>
          }
          <button type="submit" class="tr-btn tr-btn--primary" [disabled]="busy()">{{ busy() ? 'Signing in…' : 'Sign in' }}</button>
        </form>
      </div>
    </div>
  `,
