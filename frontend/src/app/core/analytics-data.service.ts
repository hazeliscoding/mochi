...
export class AnalyticsDataService {
  ...
  private readonly period = computed(() => {
    ...
    const compare =
      cmp === 'vs same period last year' ? 'year' : cmp === 'No comparison' ? 'none' : 'previous';
  ...
  readonly channelsRes = httpResource<ApiCountRow[]>(() =>
    this.statsUrl('sources', { extra: '&group=channels' }),
  );
  readonly referrersRes = httpResource<ApiCountRow[]>(() =>
    this.statsUrl('sources', { extra: '&group=referrers' }),
  );
  ...
  readonly siteOptions = computed(() =>
    (this.sitesRes.value() ?? []).map((s) => ({ id: s.site.id, domain: s.site.domain })),
  );
  ...
  readonly metrics = computed<MetricCard[]>(() => {
    ...
    return [
      {
        label: 'Visitors',
        value: fmt(cur.visitors),
        delta: pctDelta(cur.visitors, cmp?.visitors),
        dir: dirOf(cur.visitors - (cmp?.visitors ?? 0)),
        tip: METRIC_TIPS['Visitors'],
      },
      {
        label: 'Pageviews',
        value: fmt(cur.pageviews),
        delta: pctDelta(cur.pageviews, cmp?.pageviews),
        dir: dirOf(cur.pageviews - (cmp?.pageviews ?? 0)),
        tip: METRIC_TIPS['Pageviews'],
      },
      {
        label: 'Views per visitor',
        value: cur.viewsPerVisitor.toFixed(1),
        delta:
          cmp && Math.abs(vpvDiff) >= 0.05 ? signed(Math.abs(vpvDiff).toFixed(1), vpvDiff) : '',
        dir: dirOf(vpvDiff),
        tip: METRIC_TIPS['Views per visitor'],
      },
      {
        label: 'Bounce rate',
        value: Math.round(cur.bounceRatePct) + '%',
        delta:
          cmp && Math.abs(bounceDiff) >= 0.05
            ? signed(Math.abs(bounceDiff).toFixed(1) + ' pt', bounceDiff)
            : '',
        dir: dirOf(bounceDiff),
        tip: METRIC_TIPS['Bounce rate'],
      },
      {
        label: 'Avg visit duration',
        value: fmtDur(cur.avgDurationSec),
        delta: cmp && durDiff !== 0 ? signed(Math.abs(durDiff) + 's', durDiff) : '',
        dir: dirOf(durDiff),
        tip: METRIC_TIPS['Avg visit duration'],
      },
    ];
  });

  readonly series = computed(() => (this.timeseriesRes.value()?.points ?? []).map((p) => p.value));
  readonly prevSeries = computed(() =>
    (this.timeseriesRes.value()?.compare ?? []).map((p) => p.value),
  );
  ...
  readonly browserTop = computed(() =>
    (this.devicesRes.value()?.browsers ?? [])
      .slice(0, 3)
      .map((r) => ({ name: r.name, pct: Math.round(r.pct) })),
  );
  readonly osTop = computed(() =>
    (this.devicesRes.value()?.os ?? [])
      .slice(0, 3)
      .map((r) => ({ name: r.name, pct: Math.round(r.pct) })),
  );
  ...
  readonly deviceClasses = computed(() => {
    ...
    return order.map((name) => {
      ...
      return {
        name,
        pct: r ? Math.round(r.pct) : 0,
        val: fmt(r?.count ?? 0),
        color: DEVICE_COLORS[name],
      };
    });
  });
  readonly browserRows = computed<BarRow[]>(() =>
    toBarRows(this.devicesRes.value()?.browsers ?? []),
  );
  ...
  updateSite(
    id: string,
    body: { name?: string; timezone?: string; retention?: string },
  ): Observable<ApiSite> {
  ...
  createGoal(
    siteId: string,
    body: { name: string; type: string; target: string },
  ): Observable<ApiGoal> {
  ...
  readonly collectedItems: [string, string][] = [
    ['Page URL', 'the page that was viewed'],
    ['Referrer domain', 'where the visit came from'],
    ['Country', 'derived from IP, then the IP is discarded'],
    ['Device class', 'desktop, mobile, or tablet'],
    ['Browser & OS family', 'e.g. Firefox on Linux, no versions'],
    ['Custom events', 'names and counts you choose to send'],
  ];

  readonly notCollectedItems: [string, string][] = [
    ['IP addresses', 'used transiently, never written to disk'],
    ['Cookies or device IDs', 'no identifiers of any kind'],
    ['Names, emails, accounts', 'no personal information'],
    ['Precise location', 'nothing below country level by default'],
    ['Click paths per person', 'no individual timelines'],
    ['Anything cross-site', 'no shared profiles between websites'],
  ];
}
