...
public sealed class EfRollupReader(MochiDbContext db) : IRollupReader
{
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<DailyPageRow>> PagesAsync(SiteId siteId, DateOnly from, DateOnly to, CancellationToken ct = default)
        => await db.PageRows.Where(r => r.SiteId == siteId.Value && r.Date >= from && r.Date <= to)
            .Select(r => new DailyPageRow(r.Path, r.Visitors, r.Pageviews, r.Entries, r.Exits, r.BouncedSessions, r.TotalDurationSec))
            .ToListAsync(ct);

    /// <inheritdoc />
    public async Task<IReadOnlyList<DailySourceRow>> SourcesAsync(SiteId siteId, DateOnly from, DateOnly to, CancellationToken ct = default)
        => await db.SourceRows.Where(r => r.SiteId == siteId.Value && r.Date >= from && r.Date <= to)
            .Select(r => new DailySourceRow(
                (Channel)r.Channel,
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<DailyGeoRow>> GeoAsync(SiteId siteId, DateOnly from, DateOnly to, CancellationToken ct = default)
        => await db.GeoRows.Where(r => r.SiteId == siteId.Value && r.Date >= from && r.Date <= to)
            .Select(r => new DailyGeoRow(r.Country, r.Visitors))
            .ToListAsync(ct);

    /// <inheritdoc />
    public async Task<IReadOnlyList<DailyDeviceRow>> DevicesAsync(SiteId siteId, DateOnly from, DateOnly to, CancellationToken ct = default)
        => await db.DeviceRows.Where(r => r.SiteId == siteId.Value && r.Date >= from && r.Date <= to)
            .Select(r => new DailyDeviceRow((DeviceClass)r.DeviceClass, r.Browser, r.Os, r.Visitors))
            .ToListAsync(ct);

    /// <inheritdoc />
    public async Task<IReadOnlyList<DailyEventRow>> EventsAsync(SiteId siteId, DateOnly from, DateOnly to, CancellationToken ct = default)
        => await db.EventRows.Where(r => r.SiteId == siteId.Value && r.Date >= from && r.Date <= to)
            .Select(r => new DailyEventRow(r.EventName, r.Path, (Channel)r.Channel, r.Total, r.UniqueVisitors))
            .ToListAsync(ct);
}
